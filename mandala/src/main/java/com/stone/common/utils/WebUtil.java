...
import com.stone.common.Constants;
...
public final class WebUtil {
	...
	/** 获取当前用户所属系统身份信息ID */
	/*public static final String getCurrentIdentity() {
	    Subject currentUser = SecurityUtils.getSubject();
	    if (null != currentUser) {
	        Session session = currentUser.getSession();
	        if (null != session) {
	            return (String) session.getAttribute(Constants.CURRENT_IDENTITY);
	        }
	    }
	    return null;
	}*/
	
	/** 获取当前用户所属系统身份对应实体信息ID */
	/*public static final String getCurrentEntityId() {
	    Subject currentUser = SecurityUtils.getSubject();
	    if (null != currentUser) {
	        Session session = currentUser.getSession();
	        if (null != session) {
	            return (String) session.getAttribute(Constants.CURRENT_ENTITY_ID);
	        }
	    }
	    return null;
	}*/
	...
	/*public static boolean isValidateCodeLogin(String useruame, boolean isFail){
		int loginFailNum = 0;
		String loginFailNumStr = JedisUtil.hget("LOGINFAILNUM", useruame);
		try {
		...
		if (isFail){
			loginFailNum++;
			JedisUtil.hset("LOGINFAILNUM", useruame, loginFailNum + "");
		}
